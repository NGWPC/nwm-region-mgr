...
1) all pairing methods (except for URF) employ a Principal Component Analysis (PCA) to remove correlation
  among the basin attributes and reduce the dimentionality of the similarity problem
...
df_attr_all = pd.read_csv(config["inputs"]["file_attrs_data"])
df_attr_all["tag"] = "receiver"

# determine which catchments are donors (based on the donor gage list and crowsswalk table)
gages = (
    pd.read_csv(config["inputs"]["file_donor_list"], header=None, dtype="str")
    .iloc[:, 0]
    .tolist()
)
cwt = pd.read_csv(config["inputs"]["file_crosswalk"], dtype={"gages": "str"})
donors_all = cwt.loc[cwt["gages"].isin(gages)]["id"].tolist()

# the remaining catchments are receivers
df_attr_all.loc[df_attr_all["id"].isin(donors_all), "tag"] = "donor"
receivers_all = df_attr_all.loc[df_attr_all["tag"] == "receiver"]["id"].tolist()

print("Total number of catchments: " + str(df_attr_all.shape[0]))
print("Total number of donor catchments: " + str(len(donors_all)))
print("Total number of receiver catchments: " + str(len(receivers_all)))
print(
    "Total number of calibration basins: "
    + str(len(set(cwt.loc[cwt["gages"].isin(gages)]["gages"].tolist())))
)

# sort the rows so that donors are at the top
df_attr_all = df_attr_all.sort_values(by="tag")

# detemine whether the catchment is snowy (as snowy and non-snowy catchments are processed separately)
df_attr_all["snowy"] = df_attr_all["snow_frac"].apply(
    lambda x: True if x >= config["pars"]["general"]["minSnowFrac"] else False
)
...
config["attrs"]["all"] = [
    x for x in df_attr_all.columns.tolist() if x not in config["non_attr_cols"]
]

# compute spatial distance between all receivers and donors if not already computed
f1 = config["inputs"]["file_distance"]
if os.path.isfile(f1):
    dist_spatial0 = pd.read_csv(f1, index_col=0)
else:
    dist_spatial0 = utils_algo.calculate_spatial_distance(
        shp_file=config["inputs"]["file_hydrofab"],
        donors=donors_all,
        receivers=receivers_all,
    )
    dist_spatial0.to_csv(f1, index=True, header=True)
...
for func1 in funcs:
    for scenario in scenarios:
        ...
        df_donor_all = functions[func1].func(
            config, df_attr_all, scenario, dist_spatial0, func1
        )
        print(
        ...
        df_donor_all.to_csv(outfile, index=False, float_format="%.3f")
