...
def identify_donor_slow (rec1, config, method, dfAttr, dfAttrAll, dist_spatial, distAttr0, run1):
    ...
    while buffer < config['general']['max_spa_dist'] -100 :
    ...
    if len(donors1)>0:   
        ...
        # assign donor
        dfDonor = pd.concat((dfDonor, utils_algo.assign_donors(run1, donor1, [rec1], config, dist1, dist_spatial, dfAttrAll)),axis=0) 
    ...
    if len(donors1)==0 and run1=="base":                       
        ...
        # assign donor
        dfDonor = pd.concat((dfDonor, utils_algo.assign_donors("proximity", donor0, [rec1], config, dist0, dist_spatial, dfAttrAll)),axis=0) 
