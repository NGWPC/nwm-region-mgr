...
def func(config, dfAttrAll, dist_spatial, method):
    ...
    while True:
        ...
        dfAttr = utils_algo.get_valid_attrs(recs0, recs, dfAttr0, attrs1, config)

        # apply principal component analysis
        myscores, _ = utils_algo.apply_pca(dfAttr.drop(config['non_attr_cols'], axis=1))
        ...
        for snow1 in np.unique(dfAttr['snowy']):
            ...
            dfAttr1 = dfAttr[dfAttr['snowy']==snow1]
            scores1 = myscores[(dfAttr['snowy']==snow1).values]
            donors = dfAttr1[dfAttr1['is_donor']]['divide_id'].tolist()
            receivers = dfAttr1[~dfAttr1['is_donor']]['divide_id'].tolist()
            ...
            while dfDonorSnow.shape[0] < len(recs1):
                ...
                results = Parallel(n_jobs=njob)(delayed(identify_donor_by_cluster)(donors, ll, labels, receivers, dfDonorAll, dfDonorSnow, config, method, scores1, dist_spatial, dfAttrAll, label_done)   for ll in label_rec) 
...
#
def identify_donor_by_cluster(donors, ll, labels, receivers, dfDonorAll, dfDonorSnow, config, method, scores1, dist_spatial, dfAttrAll, label_done):
    ...
    if dfDonorSnow.shape[0] > 0:
        recs2 = [x for x in recs2 if x not in dfDonorSnow['divide_id'].tolist()]
    if dfDonorAll.shape[0] > 0:
        recs2 = [x for x in recs2 if x not in dfDonorAll['divide_id'].tolist()]
    ...
    if len(donors1)>0:
        ...
        if len(donors1) > config['n_donor_max']:
            ...
            if (method in ['kmeans','kmedoids']):
                ...
                if method == 'kmeans':
                    fit1 = KMeans(init=config['init'], n_clusters=2, n_init=config['n_init'], max_iter=config['n_iter_max'], random_state=42).fit(mydata1)
                elif method == 'kmedoids':
                    fit1 = KMedoids(init=config['init'], n_clusters=2, max_iter=config['n_iter_max'], random_state=42).fit(mydata1)    

            elif method == 'hdbscan':
                fit1 = hdbscan.HDBSCAN(min_samples=11,min_cluster_size=config['min_cluster_size'],allow_single_cluster=False).fit(mydata1)

            elif method == 'birch':
                # explore a range of threshold values to identify a proper threshold parameter for BIRCH                    
                for thresh in np.arange(config['min_thresh'],config['max_thresh']+0.1,0.1):
                    ...
                    while kk < config['max_resample']:
                        ...
                        fit1 = Birch(branching_factor=config['branching_factor'],n_clusters=None,
            ...
            if len(l1) > 0:                                
                ...
                dfDonor= pd.concat((dfDonor, utils_algo.assign_donors('main',donors1, recs3, config, None, dist_spatial, dfAttrAll)),axis=0)
    ...
    # for receivers in clusters without donors, chooses from all donors based on spatial proximity        
        ...
        dfDonor = pd.concat((dfDonor, utils_algo.assign_donors('proximity', donors, recs2, config, None, dist_spatial, dfAttrAll)),axis=0)            
        labels1[labels == ll] = label_done  
