...
def func(config, df_attr_all, scenario, dist_spatial, method):
    ...
    while True:
        ...
        if df_donor_all.shape[0] > 0:
            recs = np.unique(df_donor_all["id"])
        ...
        for snow1 in np.unique(df_attr["snowy"]):
            ...
            df_attr1 = df_attr.query("snowy==@snow1")
            scores1 = myscores[(df_attr["snowy"] == snow1).values]
            donors = df_attr1.query("tag=='donor'")["id"].tolist()
            receivers = df_attr1.query("tag=='receiver'")["id"].tolist()

            # receivers to be processed in this round
            recs1 = receivers.copy()
            if df_donor_all.shape[0] > 0:
                recs1 = [x for x in recs1 if x not in df_donor_all["id"].tolist()]
            ...
            if df_donor_all.shape[0] > 0:
                labels[
                    [
                        ...
                        for x in receivers
                        if x in df_donor_all["id"].tolist()
                    ]
                ] = label_done
            ...
            while df_donor_snow.shape[0] < len(recs1):
                ...
                results = Parallel(n_jobs=njob)(
                    delayed(identify_donor_by_cluster_slow)(
                        donors,
                        ll,
                        labels,
                        receivers,
                ...
                for i1 in range(njob):
                    ...
                    # print("labels_temp: " + str(np.unique(labels_temp)))
                    ...
                    if (
                        ...
                        if df_donor_snow.shape[0] == 0:
                            ...
                            recs2 = [
                                x
                                for x in recs1
                                if x not in df_donor_snow["id"].tolist()
                            ]
                        if len(recs2) > 0:
                            print(
...
#
def identify_donor_by_cluster_slow(
    donors,
    ll,
    labels,
    receivers,
    ...
    if df_donor_snow.shape[0] > 0:
        recs2 = [x for x in recs2 if x not in df_donor_snow["id"].tolist()]
    if df_donor_all.shape[0] > 0:
        recs2 = [x for x in recs2 if x not in df_donor_all["id"].tolist()]
    ...
    if len(donors1) > 0:
        ...
        if len(donors1) > config["pars"][method]["nDonorMax"]:
            ...
            if method in ["kmeans", "kmedoids"]:
                ...
                fit1 = hdbscan.HDBSCAN(
                    min_samples=11,
                    min_cluster_size=config["pars"][method]["minClusterSize"],
                ...
                for thresh in np.arange(
                    config["pars"][method]["minThresh"],
                    config["pars"][method]["maxThresh"] + 0.1,
                    0.1,
                ):
                    ...
                    while kk < config["pars"][method]["maxResample"]:
                        ...
                        fit1 = Birch(
                            branching_factor=config["pars"][method]["branching_factor"],
                            n_clusters=None,
                            threshold=thresh,
                        ).fit(mydata2)
